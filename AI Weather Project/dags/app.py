...
def render_hero(df: pd.DataFrame, latest: pd.Series, use_fahrenheit: bool) -> None:
    ...
    with col_right:
        ...
        with sub:
            ...
            c1.metric("Humidade", f"{latest.get('humidity', 'N/A')}%")
            c2.metric("Vento", f"{latest.get('wind_speed', 'N/A')} km/h")
            c3.metric("Pressão", f"{latest.get('pressure', 'N/A')} hPa")
            c4.metric("Sensação Termica", f"{feels_display:.0f}{unit}")
        with sub:
            # Hourly Weather Forecast Chart
            if not df.empty:
                # Resample data to hourly averages
                hourly_data = df.set_index("collection_timestamp").resample("h").agg({
                    "humidity": "mean",
                    "temperature_c": "mean",
                    "weather_main": "first",
                    "weather_icon": "first"
...
def main():
    ...
    default_end = datetime.combine(today + timedelta(days=1), datetime.max.time())
